...
describe('GET /api/posts', () => {
  it('should return all posts of author ID 2 in specific order with sortBy by no direction.', async () => {
    const token = makeToken(2);
    const res = await request(app)
      .get('/api/posts')
      .set('x-access-token', token)
      .query({ authorIds: '2', sortBy: 'likes' })
      .send();
    expect(res.body).toEqual({
      posts: [
        {
...
describe('GET /api/posts', () => {
  it('should return all posts of author ID 2 in specific order with sortBy and direction.', async () => {
    const token = makeToken(2);
    const res = await request(app)
      .get('/api/posts')
      .set('x-access-token', token)
      .query({ authorIds: '2', sortBy: 'reads', direction: 'desc' })
      .send();
    expect(res.body).toEqual({
      posts: [
        {
...
describe('PATCH /api/posts/:postId', () => {
  it('should not update authorId property of a post if not included in the request.', async () => {
    const token = makeToken(1);
    const postId = 1;
    const res = await request(app)
      .patch(`/api/posts/${postId}`)
